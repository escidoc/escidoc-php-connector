...
class ContextHandler extends OMBase{
    /**
     ...
     * @return SearchRetrieveResponse
     */
    public function retrieveContexts($criteria){
    ...
    public function open($resource){
        ...
        return new TaskResult($this->send($this->url."/".$id."/open",'POST',$body));
    }
    /**
     * @param GenericResource $resource
     * @param array $resource array("id"=>"escidoc....","last-modification-date"=>"2011...time...")
    ...
    public function close($resource){
        ...
        $body='<param last-modification-date="'.$lastmodificationdate.'"/>';  
        return new TaskResult($this->send($this->url."/".$id."/close",'POST',$body));
    }
